...
body {
  ...
  min-height: 100vh;
	...
	%>
    <h2 align="center" >Prodotti più venduti</h2><br><br>
	
	<div class="container-fluid">
  <div class="row">
    <div class="col	">
    <div class="col-7">
    <form action="./ShowBestSellers" name="timeForm" id="timeForm" method="POST">
            	<c:if test="${filter eq 0}">
            		<select name="timeSelection" required class="custom-select" id="timeSelection" >
			        <option selected value="0">Sempre</option>
			        <option value="1">Ultima settimana</option>
			        <option value="2">Ultimo mese</option>
			    </select>
            	</c:if>
            	<c:if test="${filter eq 1}">
            		<select name="timeSelection" required class="custom-select" id="timeSelection" >
			        <option value="0">Sempre</option>
			        <option selected value="1">Ultima settimana</option>
			        <option value="2">Ultimo mese</option>
			    </select>
            	</c:if>
            	<c:if test="${filter eq 2}">
            		<select name="timeSelection" required class="custom-select" id="timeSelection" >
			        <option value="0">Sempre</option>
			        <option value="1">Ultima settimana</option>
			        <option selected value="2">Ultimo mese</option>
			    </select>
            	</c:if>
			    </form>
			    <br><br>
            ...
            <div class="col-7">
                ...
                <div class="panel list-group" >
                 ...
                 <a href="#"  class="list-group-item" >Vendite <span class="label label-info"></span> <span class="glyphicon glyphicon-envelope pull-right"></span></a>
                ...
                <a href="./ShowMostClicked" class="list-group-item" >Più cliccati <span class="glyphicon glyphicon-tag pull-right"></span></a>
                 <a href="./ShowMostLiked" class="list-group-item" >Feedback<span class="glyphicon glyphicon-tag pull-right"></span></a>
    ...
    <div class="col">
      <p><b>Totale prodotti venduti:</b> <%= request.getAttribute("total") %> </p><br><br>
      <p><b>Prodotto più venduto:</b>  ${ itemList[0].name } </p><br><br>
      <c:set var="size" value="${fn:length(itemList)}" />
       <p><b>Prodotto meno venduto:</b>  ${ itemList[size-1].name }</p>
...
var colors=[];
var data=[];
<c:forEach items="${itemList}" var="id">
array.push("${id.name}"); 
colors.push(getRandomColor());
</c:forEach>
<c:forEach items="${orderData}" var="i">
data.push("${i}");
</c:forEach>
...
$(document).ready(function() {
	  $('#timeSelection').on('change', function() {
	     document.forms['timeForm'].submit();
	  });
	});
</script>
