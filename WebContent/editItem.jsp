...
body {
  ...
  min-height: 100vh;
 	...
 	<body>
		...
		<form action="SaveProduct" method="post" enctype="multipart/form-data" accept-charset="UTF-8" onsubmit="button1id.disabled=true; deleteButton.disabled=true; addPhoto.disabled=true; return true;">
 				...
 				<div class="row">
    				<div class="col">
     					<div class="form-horizontal">
 							<div class="form-group">
  								<div class="col-md-8">
  									<input  class="form-control input-md" id="textinput" type="text" value="${product.ID}" placeholder="ID prodotto: ${product.ID}" disabled>
  									<input name="productID" hidden="true" value="${product.ID}">
  								</div>	
							</div>
							<div class="form-group">
								<div class="col-md-8">
									<input name="productName" class="form-control input-md" id="textinput" type="text" value="${product.name}" placeholder="Nome prodotto" required>
								</div>
							</div>
							<div class="form-group">
								<div class="col-md-8">
									<input name="price" class="form-control input-md" id="textinput" pattern="^[0-9.,]+$" value="${product.price}" type="text" placeholder="Prezzo (&euro;)" required>
								</div>
							</div>
							<div class="form-group">
  								<div class="col-md-8">
  								...
  								<div class="col-md-8">
   									<div class="custom-control custom-checkbox">
   										<c:if test="${product.onSale}">
   											<input type="checkbox" name="onSale" checked class="custom-control-input" id="customCheck1">
   										</c:if>
  										<c:if test="${!product.onSale}">
    				...
    				<div class="col">
     					<div class="form-horizontal">
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="color1" class="form-control input-md" id="textinput" type="text" value="${product.colors[0]}" placeholder="Colore principale" required>
  								</div>
							</div>
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="color2" class="form-control input-md" id="textinput" type="text" value="${product.colors[1]}" placeholder="Colore #2 (opzionale)">
  								</div> 
							</div>
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="color3" class="form-control input-md" id="textinput" type="text" value="${product.colors[2]}" placeholder="Colore #3 (opzionale)">
  								</div>
							</div>
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="color4" class="form-control input-md" id="textinput" type="text" value="${product.colors[3]}" placeholder="Colore #4 (opzionale)">
  								</div>
							</div>
						</div>
    				</div>
    				<div class="col">
    					<div class="form-horizontal">
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="tag1" class="form-control input-md" id="textinput" type="text" value="${product.tag[0]}" placeholder="Brand (Consigliato)">
  								</div>
							</div>
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="tag2" class="form-control input-md" id="textinput" type="text" value="${product.tag[1]}" placeholder="Tag #2 (opzionale)">
  								</div>
							</div>
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="tag3" class="form-control input-md" id="textinput" type="text" value="${product.tag[2]}" placeholder="Tag #3 (opzionale)">
  								</div>
							</div>
							<div class="form-group">
  								<div class="col-md-8">
  									<input name="tag4" class="form-control input-md" id="textinput" type="text" value="${product.tag[3]}" placeholder="Tag #4 (opzionale)">
  								</div>
							</div>
						</div>
    				</div>
  				</div>
  				<input type="text" hidden="true" id="firstPhoto" name="firstPhoto" value="${product.path[0]}" >
				<h3><label for="exampleFormControlTextarea1">Descrizione</label></h3>
    			<textarea name="description" class="form-control" id="exampleFormControlTextarea1"  rows="9" >${product.descriptionHTML}</textarea>
    			<div class="form-group" >
    				<div class="space"></div>
   					<div class="container">
  						<div class="row">
    						<div class="col">
            					<button type="button" id="addPhoto"class="btn btn-outline-primary" data-toggle="modal" data-target="#exampleModal">Modifica foto</button>
    						</div>
    						<!-- Modal -->
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-body">
											<!-- Foto 1 -->
											<p>Se <i>Cancella foto esistenti</i> è selezionato, le foto caricate sostituiranno le precedenti.</p>
											...
											<div class="custom-control custom-checkbox">
												<input type="checkbox" name="deletePhotos" class="custom-control-input" id="deletePhotos">
												<label class="custom-control-label" for="deletePhotos" >Cancella foto esistenti</label>
											</div>
										</div>
								...
								<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								  <div class="modal-dialog" role="document">
								    <div class="modal-content">
								      ...
								      <div class="modal-body">
								        Sei sicuro di voler eliminare il prodotto?<br><br>
								        <div class="custom-control custom-checkbox">
												<input type="checkbox" name="deleteFromDB" class="custom-control-input" id="deleteFromDB">
												<label class="custom-control-label" onclick="showDanger()" for="deleteFromDB" >Cancella il prodotto anche dal database</label>
											</div>
											<br><br>
											 <div  id="dangerAlert" style="width:70%; text-align:left; display:none" class="alert alert-danger alert-autocloseable-danger">
	  											<b>ATTENZIONE:</b> Cancellare il prodotto dal database comporterà i seguenti problemi:
	   
												  <ul>
												  <li>Gli utenti non saranno mai più in grado di visualizzare e lasciare feedback per
												  il prodotto eliminato.</li>
												<li>Le statistiche perderanno il riferimento al prodotto eliminato.</li>
												  </ul>
												  
												  <b>L'eliminazione del prodotto dal database è una scelta irreversibile.<br>
												  Si consiglia di non selezionare questa spunta.</b>
											</div>
								      </div>
								      <div class="modal-footer">
								        <button type="button" class="btn btn-secondary" data-dismiss="modal">Annulla</button>
								        <a id="buttonDelete" href="./DeleteProduct?productID=${product.ID}"><button type="button" class="btn btn-primary">Conferma</button></a>
								      </div>
								    </div>
								  </div>
								</div>
    						...
    						<div class="col">  
     							<button type="submit" name="button1id" class="btn btn-primary"  id="button1id">Conferma modifiche</button>
   							</div>
 	 					</div>
					</div>
					<br> 
...
function showDanger() {
	var checkBox = document.getElementById("deleteFromDB");
	  var text = document.getElementById("dangerAlert");
	  if (checkBox.checked == true){
	    text.style.display = "none";
	    var a = document.getElementById('buttonDelete');
	    a.href = "./DeleteProduct?productID=${product.ID}"
	  } else {
	     text.style.display = "block";
	     var a = document.getElementById('buttonDelete');
		    a.href = "./DeleteProduct?productID=${product.ID}&deleteFromDB=true"
	  }
}
  </script>
  </body>
